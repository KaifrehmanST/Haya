...
static std::once_flag g_backend_load_once;
...
LLMInference::loadModel(const char *model_path, float minP, float temperature, bool storeChats, long contextSize,
    ...
    // Load dynamic backends once per process. Re-loading them on every model switch
    // can keep native resources alive longer than expected and cause memory pressure.
    ...
    _messages.clear();
    addChatMessage("You are a helpful, concise AI assistant.", "system");
...
LLMInference::stopCompletion() {
    if (_storeChats) {
        ...
        } else if (!_messages.empty() && std::strcmp(_messages.back().role, "user") == 0) {
            auto lastMessage = _messages.back();
            free(const_cast<char *>(lastMessage.role));
            free(const_cast<char *>(lastMessage.content));
            _messages.pop_back();
        }
    }
    _response.clear();
