...
package com.monicauditya.smollm
...
class SmolLM {
    ...
    object DefaultInferenceParams {
        ...
        val maxMobileContextSize: Long = 4096L
    ...
    suspend fun load(modelPath: String, params: InferenceParams = InferenceParams()) =
        withContext(Dispatchers.IO) {
            ...
            val rawModelContextSize = ggufReader.getContextSize() ?: DefaultInferenceParams.contextSize
            val modelContextSize = minOf(rawModelContextSize, DefaultInferenceParams.maxMobileContextSize)
    ...
    fun getResponseAsFlow(query: String): Flow<String> = flow {
        val modelPtr = synchronized(nativeLock) {
            verifyHandle()
            stopRequested = false
            activeGenerations += 1
            nativePtr
        ...
        try {
            startCompletion(modelPtr, query)
            var piece = completionLoop(modelPtr)
            while (piece != "[EOG]") {
                if (stopRequested) {
                    throw CancellationException("Generation stopped")
                }
                emit(piece)
                piece = completionLoop(modelPtr)
            }
    ...
    private external fun requestStop(modelPtr: Long)
